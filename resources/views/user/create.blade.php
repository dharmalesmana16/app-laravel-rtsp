            ...
            <form x-data="{
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <x-text-input id="name" class="w-full" x-model="name" placeholder="Nama lengkap" />
                    </div>
                    <div>
                        <x-input-label for="email" value="Email" />
                        <x-text-input type="email" id="email" class="w-full" x-model="email" placeholder="email@example.com" />
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <x-input-label for="password" value="Password" />
                            <x-text-input type="password" id="password" class="w-full" x-model="password" placeholder="Min 8 karakter" />
                        </div>
                        <div>
                            <x-input-label for="password_confirmation" value="Konfirmasi Password" />
                            <x-text-input type="password" id="password_confirmation" class="w-full" x-model="password_confirmation" placeholder="Ulangi password" />
                        </div>
                    </div>
                    <div>
    ...
    <script>
        function onCreate(e) {
            ...
            if (this.password !== this.password_confirmation) {
                Swal.fire({ title: "Password tidak cocok!", icon: "error" });
                return
            }
